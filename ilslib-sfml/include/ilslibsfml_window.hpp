...
namespace ILSLibSFML
{
	...
	*//*------------------------------------------------------------------------------*/
	...
	public:
		...
		virtual void newWindow(const sf::VideoMode& vMode, const std::string& title, const bool fullScreen);
		virtual void close(bool closeOnUpdate = false);
		virtual void pollEvents(const std::string& root, unsigned int maxEvents = 0);
		virtual void parseEvents(const std::string& root, unsigned int maxEvents = 0);
		virtual void updateDrawableMap(const std::string& root);
