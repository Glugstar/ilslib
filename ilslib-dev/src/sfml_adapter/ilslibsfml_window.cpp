#include "../../include/sfml_adapter/ilslibsfml_window.hpp"
...
namespace ILSLibSFML
{
	...
	void Window::newWindow(const sf::VideoMode& vMode, const std::string& title)
	{
		close();
	...
	void Window::pollEvents(unsigned int maxEvents)
	{
		if(isOpen() == false)
			return;
		...
		parser.parseSFMLEvent(nullptr, eventTime, blockManager, eventQueue);
        ...
        while(true)
        {
			...
			parser.parseSFMLEvent(&event, eventTime, blockManager, eventQueue);
	...
	void Window::parseEvents(unsigned int maxEvents)
	{
		unsigned int eventCounter = 0;
        while(true)
        {
	...
	void Window::updateDrawableMap()
	{
		if(isOpen() == false)
			return;
		
		blockManager.calculateDPInfo(&drawableMap, videoMode.width, videoMode.height);
	}
