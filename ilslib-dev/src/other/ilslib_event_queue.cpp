...
namespace ILSLib
{
	...
	void EventQueue::parseNextEvent()
	{
		...
		if(blockManager != nullptr)
			basicBlock = (*blockManager)[*path];
		if(basicBlock != nullptr)
			listener = basicBlock->getListenerManager().getListener(listenerID);
		
		if(listener != nullptr && event != nullptr && blockManager != nullptr && basicBlock != nullptr)
